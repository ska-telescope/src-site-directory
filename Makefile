...
MONGO_USERNAME= root
MONGO_PORT= 27017
MONGO_HOST ?= mongo.$(KUBE_NAMESPACE).svc.$(CLUSTER_DOMAIN)
...
PYTHON_VARS_BEFORE_PYTEST ?= PYTHONPATH=.:./src CLUSTER_DOMAIN=$(CLUSTER_DOMAIN) KUBE_NAMESPACE=$(KUBE_NAMESPACE) DISABLE_AUTH=$(DISABLE_AUTH) MONGO_USERNAME=$(MONGO_USERNAME) MONGO_PASSWORD=$(MONGO_PASSWORD) MONGO_HOST=$(MONGO_HOST) MONGO_PORT=$(MONGO_PORT) MONGO_DATABASE=$(MONGO_DATABASE)
